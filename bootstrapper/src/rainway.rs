...
/// launches the Rainway service (Rainway.Service)
    ...
    if let Some(process_list) = get_processes() {
        for process in process_list {
            if process.name() == "Rainway.exe"
                || process.name() == "Rainway.Daemon.exe"
                || process.name() == "Rainway.Service.exe"
            {
                if process.kill() {
